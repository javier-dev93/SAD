...
# Creamos un for loop que itere sobre las columnas que hemos especificado, y las recodifique como factor, especificando los nuevos factores
...
```{r}
# Creamos un "subset" a partir de nuestro conjunto de datos, seleccionando las filas que cumplen las dos (indicado con el símbolo &) condicones (Dx.cancer=1 y Dx.HPV=1). Después, para saber el total de pacientes que cumplen ambas condiciones solo necesitamos saber el número de filas del conjunto (nrow)
...
En primer lugar, y dada la importancia de la edad en este tipo de estudios que ya hemos comentado, podemos realizar un histograma de esta variable, añadiéndole la línea de densidadpara ver cómo se distribuye.
...
Para evaluar de un vistazo los datos de las pruebas diagnósticas así como el diagnóstico final, podemos representarlas todas estas variables a la vez utilizando un barplot agrupado:
...
Para observar la relación entre dos variables numéricas, podemos realizar una representación en forma de dotplot, como haremos a modo de ejemplo para evaluar a simple vista si existe una relación entre la edad de la paciente y la edad a la que tuvo lugar el primer encuentro sexual, e identificar posibles variaciones que se relacionen con diferencias culturales entre generaciones.
...
dbinom(3,200,p_hpv)
```
Mediante la función shadeDist de la librería fastGraph podemos dibujar las distribuciones, a la hora de
dibujar la distribución la escala del eje x será de 20 mujeres, como veremos a partir de 10 mujeres la probabilidad tiende a cero.  
...
¿Cuál es la probabilidad de que mas de 10 mujeres sean diagnosticadas de cancer y esten diagnosticadas de HPV?

La probabilidad de encontrar más de 10 casos, será la probabilidad acumulada de encontrar más de 10
mujeres diagnosticadas de cancer y HPV. Tendríamos que calcular la probabilidad acumulada de X mujeres, donde X>10 (4,5,6,7,8…), a medida que aumentamos el número de mujeres, la probabilidad de cada valor se aproximara a cero.
...
Realizamos una simulación generando 1500 valores aleatorios suponiendo que para 858 mujeres observadas, hay una probabilidad de 0.0186669, de que estás sean diagnosticada de cancer y HPV.
...
```{r}
#Realizamos gráficos de dispersión de todas la variables.
#pairs(datos_reg)
```
...
El modelo de regresión tiene una R-squared de 0.2768, tenemos un ajuste de casi un 27%, sería un coeficiente bajo Si bien,  tenemos algunos puntos lejos de la linea de regresión, la mayoría de los puntos tienen una distribución uniforme en las proximidades de la linea.
...
summary(RegLinMul_p)
```
Podemos observar que el R-squared  0.3506, mejor respecto al cálculo anterior. Por lo tanto hemos mejorado el ajuste mediante la regresión lineal múltiple.
...
Podemos ver que parece haber una diferencia bastante pequeña entre la edad del primer encuentro sexual entre las pacientes que sí fueron diagnosticadas con cáncer y las que no, que es mayor en el primer grupo. Podemos realizar una representación gráfica para vez la distribución de los datos:
...
Estás variables al tener distinta dimensión, habrá que realizar un escalado, por ejemplo, centrar las variables de forma que todas ellas tengan media 0 y desviación estándar 1.

Una vez tratado los datos, procederemos a aplicar distintas técnicas de clustering, realizaremos el análisis de dos métodos.
...
```
Podemos observar que el mejor valor que se obtiene es para k=2.
...
No queda claro el “codo” en la curva.
...
```
El mejor valor se obtiene para k=3.
...
Como se puede comprobar, agrupar con dos cluster es mejor que en 4 o en 8.

Conclusiones:

Hemos probado el modelo varias veces con distintos número de registros, partiendo de 50 ejemplos hasta 200.

La agrupación por kmeans nos da buenos resultados para dos clusters k = 2.

Conforme aumentamos el número de ejemplos a tratar, la mayoría de los ejemplos se clasifican en el cluster 1, mientras que una minoria de ejemplos se clasifican en el cluster 2.

También se puede observar que la calidas de la agregación va disminuyendo conforme aumentan los ejemplos que tratamos, en todos los casos siempre la agrupación con 2 cluster da mejor resultados.

En el cluster 1 se clasificarán .....

En el cluster 2 se clasificarán .....

-Estrategia Jerarquica

La estrategía jerarquica consiste en dados ejemplos sin etiquetar descritos por valores de atributos, obtener una jerarquía de clases que incluya cada una a un subconjunto de los ejemplos de entrenamiento.

Estas jerarquias se denominan dendogramas, donde cada nodo representa una clase de observaciones.

La entradas son un conjunto de ejemplos de entrenamiento descritos por un conjunto de atributos y sus valores respectivos.

Las salidas es una jerarquía de clases(dendograma).

Se dividen en dos métodos, aglomerativa y divisional.
...
Algunos ejemplos en el cluster 2 de kmeans

```{r}
data_cluster_backup[6,9]
```
