...
Finalmente, las cuatro últimas variables del dataset (Hinselmann, Schiller, Cytology y Biopsy) indican pruebas diagnósticas utilizadas frecuentemente en cáncer. Separando por pacientes diagnosticadas con cáncer, se podría un nuevo dataset con el número total de pruebas (entre 0 y 4).

```{r}
# Para crear un nuevo data.frame, especificamos las dos columnas que queremos que lo compongan. La primera, llamada diagnosis es sencillamente una copia de la columna Dx.Cancer del dataframe original. La segunda, llamada number_of_tests la obtenemos al sumar directamente las 4 columnas de diagnóstico, ya que el valor 1 se corresponde con que se ha realizado la prueba
pruebas_positivas <- data.frame("Diagnosis"=cervical_cancer$Dx.Cancer, "number_of_tests"=(cervical_cancer$Hinselmann+ cervical_cancer$Schiller+cervical_cancer$Citology+cervical_cancer$Biopsy))
...
Para conocer con más detenimiento algunas de las variables del conjunto de datos podemos realizar una serie de representaciones gráficas.

En primer lugar, y dada la importancia de la edad en este tipo de estudios que ya hemos comentado, podemos realizar un histograma de esta variable, añadiéndole la línea de densidad 
para ver cómo se distribuye.

```{r}
hist(cervical_cancer$Age, breaks=20, col="aquamarine1", xlab="Individuos 0+", ylab="Frecuencia",main="Histograma de edad",xlim=c(0,85),freq=F) # Cambiamos el número de intervalos y coloreamos el histograma de rosa claro, además de añadir títulos a los ejes y al gráfico completo y cambiar los límites del eje x para que aparezca entero
lines(density(cervical_cancer$Age),col="aquamarine4",lwd=3)
```


Barplot agrupado variables categóricas
...
~~algo relacionado con el tema de la incidencia/prevalencia?~~ 
~~se puede simular una distribución a partir de nuestros datos?~~
